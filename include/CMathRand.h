...
namespace CMathRand {
  inline void seedRand(int s) {
    COSRand::srand(s);
  }

  inline int randInRange(int rmin, int rmax) {
  ...
  inline double symmetricUnitRand() {
    return COSRand::randIn(-1.0, 1.0);
  }

  inline double randInRange(double rmin, double rmax) {
    return COSRand::randIn(rmin, rmax);
  }

  inline double noiseFade(double t) {
    return t * t * t * (t * (t * 6 - 15) + 10);
  }

  inline double noiseLerp(double t, double a, double b) {
    return a + t * (b - a);
  }
