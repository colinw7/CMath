...
bool
CMathGeom2D::
IntersectPolygon(const std::vector<CPoint2D> &points, const CBBox2D &bbox,
...
bool
CMathGeom2D::
IntersectPolygons(const int *x1, const int *y1, uint n1,
                  const int *x2, const int *y2, uint n2,
                  int **xi, int **yi, uint *ni)
{
  *xi = nullptr;
  *yi = nullptr;
  *ni = 0;
...
typedef std::vector<CutPoint>    CutPointArray;
typedef std::list<CutPointArray> CutPointArrayList;

//! intersect polygon (points) and bbox and return the result in (cpoints) if successful
bool
CMathGeom2D::
CutPolygon(const std::vector<CPoint2D> &points, const CBBox2D &bbox,
           std::vector< std::vector<CPoint2D> > &cpoints)
{
  ...
  return CutPolygons(points, points1, cpoints);
}
...
CutPolygons(const std::vector<CPoint2D> &points1, const std::vector<CPoint2D> &points2,
            std::vector<CPoint2D> &cpoints)
{
  std::vector< std::vector<CPoint2D> > cpoints1;
...
typedef std::vector<AddPoint> AddPointArray;
typedef std::list<AddPoint>   AddPointList;

//! add two polygons (points1) and (points2) and return the
//! result in (ipoints) if successful
bool
CMathGeom2D::
AddPolygons(const std::vector<CPoint2D> &points1, const std::vector<CPoint2D> &points2,
            std::vector<CPoint2D> &ipoints)
{
  ipoints.clear();
...
bool
CMathGeom2D::
PointInsideConvex(const CPoint2D &point, const std::vector<CPoint2D> &points)
{
...
bool
CMathGeom2D::
PointInsideEvenOdd(const CPoint2D &point, const std::vector<CPoint2D> &points)
{
...
bool
CMathGeom2D::
PointInsideByAngle(const CPoint2D &point, const std::vector<CPoint2D> &points)
{
              ...
              double *xi, double *yi, double *mu1, double *mu2)
{
  ...
  if (fabs(delta) < EPSILON_E6) { // parallel
    if (xi != nullptr) *xi = 0.0;
    if (yi != nullptr) *yi = 0.0;

    if (mu1 != nullptr) *mu1 = 0.0;
    if (mu2 != nullptr) *mu2 = 0.0;
  ...
  double m1 = (dx*offset_y2 - dy*offset_x2)*idelta;
  double m2 = (dx*offset_y1 - dy*offset_x1)*idelta;

  if (xi != nullptr) *xi = x11 + m1*offset_x1;
  if (yi != nullptr) *yi = y11 + m1*offset_y1;

  if (mu1 != nullptr) *mu1 = m2;
  if (mu2 != nullptr) *mu2 = m1;
...
CPolygonOrientation
CMathGeom2D::
PolygonOrientation(const int *x, const int *y, uint num_xy)
{
  int i = 2;
...
CPolygonOrientation
CMathGeom2D::
PolygonOrientation(const std::vector<CPoint2D> &points)
{
  int i = 2;
...
bool
CMathGeom2D::
PolygonIsConvex(const std::vector<CLine2D> &lines)
{
...
bool
CMathGeom2D::
PolygonIsConvex(const std::list<CLine2D> &lines)
{
  uint num_lines = lines.size();
...
PolygonIsConvex(const double *x, const double *y, int num_xy)
{
  ...
  for (int i3 = 0; i3 < num_xy; i1 = i2, i2 = i3++) {
    double dx1 = x[i2] - x[i1];
    double dy1 = y[i2] - y[i1];
    double dx2 = x[i3] - x[i2];
    double dy2 = y[i3] - y[i2];
...
bool
CMathGeom2D::
PolygonIsConvex(const int *x, const int *y, int num_xy)
{
  if (num_xy < 3)
    return false;
...
double
CMathGeom2D::
PolygonArea(const int *x, const int *y, uint num_xy)
{
  return fabs(0.5*PolygonArea2(x, y, num_xy));
}
...
double
CMathGeom2D::
PolygonArea(const std::vector<CPoint2D> &points)
{
...
double
CMathGeom2D::
PolygonArea2(const std::vector<CPoint2D> &points)
{
  uint num_points = points.size();
...
double
CMathGeom2D::
PolygonArea2(const std::list<CPoint2D> &points)
{
  double area = 0.0;
...
void
CMathGeom2D::
PolygonCentroid(const int *x, const int *y, int num_xy, int *xc, int *yc)
{
  *xc = 0;
  *yc = 0;
...
bool
CMathGeom2D::
SlicePolygonByLines(const std::vector<CPoint2D> &poly, const CLine2D &line,
                    std::vector< std::vector<CPoint2D> > &opolys)
{
  ...
  std::vector<SlicePoint> tpoints2[2];
  ...
  for (uint j = 0; j < 2; ++j) {
    ...
    std::vector<CPoint2D> opoly;

    std::vector<SlicePoint> &tpoints = tpoints2[j];
...
bool
CMathGeom2D::
PolygonLineIntersect(const std::vector<CPoint2D> &points, const CLine2D &line,
                     std::vector<CPoint2D> &ipoints)
{
  std::map<uint,CPoint2D> ipoints1;

  PolygonLineIntersect(points, line, ipoints1);

  std::map<uint,CPoint2D>::const_iterator p1, p2;
                     ...
                     double *xi, double *yi, uint *num_i)
{
  ...
  *num_i = 0;
  ...
  for (int i2 = 0; i2 < (int) nxy; i1 = i2, ++i2) {
    ...
    if (xi != nullptr) xi[*num_i] = xi1;
    if (yi != nullptr) yi[*num_i] = yi1;
...
void
CMathGeom2D::
BezierToLines(const C2Bezier2D &bezier, std::vector<CPoint2D> &points, double tol)
{
...
void
CMathGeom2D::
BezierToLines(const C3Bezier2D &bezier, std::vector<CPoint2D> &points, double tol)
{
  CBezierToLine b_to_l;
...
void
CMathGeom2D::
PointsRange(const std::vector<CPoint2D> &points, CPoint2D &min_point, CPoint2D &max_point)
{
  uint num_points = points.size();
...
void
CMathGeom2D::
PointsRange(const std::vector<CIPoint2D> &points, CIPoint2D &min_point, CIPoint2D &max_point)
{
  uint num_points = points.size();
